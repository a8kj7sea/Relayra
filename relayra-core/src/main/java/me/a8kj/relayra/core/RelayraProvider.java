...
public final class RelayraProvider implements Relayra {
    ...
    private final RelayraQueueAgent queueAgent;
    ...
    public static Relayra init(@NonNull String configName, String path) {
        ...
        if (Boolean.TRUE.equals(ConfigSchema.LOGGER_ENABLED.fetch(memory))) {
    ...
    private RelayraProvider(@NonNull PairedDataMemory<String> config, String configName) {
        ...
        this.queueAgent = new RelayraQueueAgent(this);
    ...
    public void shutdown() {
        try {
            ...
            throw new RuntimeException("Error during Relayra shutdown", e);
        }
    }
    ...
    public <T> T createCommandClient(Class<T> clientInterface) {
        try {
            ...
            throw new RuntimeException("Failed to generate command client: " + clientInterface.getName(), e);
        }
    }
    ...
    public <T> T createPublisher(Class<T> publisherInterface) {
        try {
            return new ByteBuddy()
                    ...
                    .make()
                    .load(publisherInterface.getClassLoader(), ClassLoadingStrategy.Default.INJECTION)
                    .getLoaded()
                    .getDeclaredConstructor()
                    .newInstance();
        } catch (Exception e) {
            throw new RuntimeException("Failed to generate publisher: " + publisherInterface.getName(), e);
        }
    }
